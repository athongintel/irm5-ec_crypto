...
ECDSA::ECDSA(CE_curve* curve, CE_point* pubKey, mpz_t* priKey){
	this->curve = curve;
	this->pubKey = pubKey;
	this->priKey = priKey;
}
...
int ECDSA::generateSignature(const char* message, char* signature){
	...
	do{
		...
		//compute r		
		CE_point* p1 = this->curve->mul(this->curve->g, &k);
...
bool ECDSA::verifySignature(const char* message, const char* signature){
	...
	CE_point* product = this->curve->mul(this->pubKey, &this->curve->n);			
	...
	if (checkFail){
		...
		if (mpz_cmp(r, this->curve->n)>=0 || mpz_cmp(s, this->curve->n)>=0){
			...
			CE_point* p2 = this->curve->mul(this->curve->g, &u1);
			CE_point* p3 = this->curve->mul(this->pubKey, &u2);
			CE_point* p4 = this->curve->add(p2, p3);			
